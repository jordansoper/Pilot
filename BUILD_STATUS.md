...
| 6 | Desktop app (macOS + Windows): **fully self-contained**, single signed installer that bundles its own Node runtime + `node-pty` (no system Node/pnpm/repo needed). Launchable GUI wrapping the daemon — tray/menu-bar lifecycle, pairing-QR window, settings (port/name/bind-to-Tailscale-IP), folder-access GUI for the FS allowlist, tool toggles, run-at-login | new `packages/desktop/` (Electron; reuses `startServer()` from `@pilot/cli` + `@pilot/shared` schemas). See PROJECT_PLAN §9 Phase 6 |
